...
- **Agent skill ships in-repo with one-command install.** `skill/deepsight/` now hosts the full agent skill (SKILL.md, tool-protocol + fal-gpt-image-2 references, launchd template), and `scripts/install-skill.sh` installs it into the detecting agent's skills directory (Hermes, Claude Code, Codex, or `DEEPSIGHT_SKILLS_DIR`) via `curl -fsSL https://raw.githubusercontent.com/Reality-Shifting-Tech/deepsight/main/scripts/install-skill.sh | bash` — no user steps. README leads with the one-liner so any agent reading the repo can load the skill immediately; AGENTS.md documents the new layout.
- **Synthetic eval matrix scales without network.** `eval/gen_synthetic.py` (PIL, seeded) generates 97 deterministic images: 11 base generators x 8 variants (base, rot90, fliph, flipv, bright050, bright150, jpeg50, noise) plus 10 seeded fuzz smoke images. `eval-synth` no longer needs Xcode (replaces the compiled Swift generator that made 5 images). Expectations are probe-then-lock, verified against the compiled binary: negatives (`humans_zero`, `sports_absent`, `ocr_empty`) on all 97; `ocr_contains` only on variants where OCR provably survives (flip variants emit garbage tokens, excluded); `rectangles_min` on all geometry variants. Fuzz images carry an empty `expected` block = crash-only smoke pass; `load_manifest` auto-merges the generated synthetic manifest so it can never drift by hand. The public Commons set is now a realism anchor; scaling is synthetic, zero downloads. Baseline: 110/111 images, 319/320 checks (99.7%); the lone failure is the deliberate tennis gap anchor.
